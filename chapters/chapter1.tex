...
o rozmiarach $1\m @ 1\AU$ badź $10\cm$ w zewnętrznych obszarach dysku.
...
radialnego dryfu od rozmiaru cząstek jest zróżnicowanie ich prędkości wzgledem
...
coraz to większych aglomeratów cząstek~\citep{BW08}, ale dla dużych róznic w
...
Drugim mechanizmem powodującym turbulencję gazu jest niestabilność Kelvina-
Helmholtza wzbudzana na skutek sedymentacji pyłu (Johansen et al. 2006).
Niestabilność KH zachodzi w przypadku przepływów ścinanych pojedynczego płynu,
bądź na styku dwóch płynów poruszających się z różnymi prędkościami prowadząć do
wzmocnienia drobnoskalowych zaburzenia i wzbudzenia turbulencji. W dyskach
okołogwiazdowych osiadanie pyłu prowadzi do zwiększenia stosunku pyłu do gazu w
płaszczyźnie dysku. Pył porusza się na wybranej orbicie z prędkością
keplerowską, natomiast gaz pod wpływem radialnego gradientu ciśnienia z
prędkością podkeplerowską. Duża koncentracja pyłu w płaszczyźnie powoduje, że
gaz jest „pchany” przez pył i zaczyna poruszać się szybciej niż warstwy gazu
leżące poniżej i powyżej płaszczyzny rotacji dysku. Prowadzi to do pionowego
gradientu prędkości azymutalnej gazu, co stanowi konfigurację niestabilną ze
względu na niestabilność KH. Podobnie jak w przypadku niestabilności
...
pyłu, a turbulencją.
W wyniku jednej, bądź obu, z powyższych niestabilności w dysku formuje się
warstwa pyłu o skończonej grubości. Dysk ten jest jednak zbyt rzadki, aby
wzbudziła się w nim niestabilność grawitacyjna. Jednakże uważne potraktowanie
oddziaływań między pyłem, a gazem w dyskach protoplaneternanych doprowadziło do
odkrycia tzw. niestabilności strumieniowej (Youdin \& Goodman, 2005), będącej
analogiem znanej z fizyki plazmy niestabilności strumieniowej dla dwóch,
różnoimiennych elektrycznie płynów. 
...
Dopiero niedawne prace~\cite{youdin-johansen-07,johansen-youdin-07} ujawniły
nową możliwość
rozwiązania problemu bariery metrowej dzięki odkryciu nowego rodzaju
niestabilności działającej w środowisku gazowo-pyłowym dysków
protoplanetarnych. Dzięki sprzężeniu poprzez siłę oporu aerodynamicznego,
ośrodek złożony z gazu i pyłu, dwóch składników rotujących z różnymi
prędkościami azymutalnymi, jest  podatny na tzw. {\em niestabilność
strumieniową}, znaną również pod potoczną nazwą {\em traffic jam
instability}. Jak pokazują wspomniane prace, niestabilność ta może
prowadzić do lokalnego wzrostu stosunku pyłu do gazu o czynnik $10\div100$.
Tak duży (niegrawitacyjny) wzrost gęstości pyłu może spowodować, że
obłoki pyłu stają się grawitacyjnie niestabilne i uformują planety.
Dotychczasowe badania niestabilności strumieniowej (Youdin \&
Johansen~\cite*{youdin-johansen-07} oraz Johansen \&
Youdin~\cite*{johansen-youdin-07}) były przeprowadzone w przybliżeniu lokalnym,
z pomocą magnetohydrodynamicznych symulacji numerycznych w tzw. domenie
ścinanej (ang. {\em shearing box}),   przy użyciu dość prostych modeli,
zaniedbujących np. pionową stratyfikację ośrodka gazowo-pyłowego.
...
radialnym jak oraz wertykalnym i podlegać wzajemnym zderzeniom. 
Z drugiej strony, ciała o rozmiarach

...
$\tau_f$ jest skalą czasową tarcia\footnote{ang. friction time}~\mref{eq:tauf},
...
%
Zgodnie z~pracą Trac i Pen~\cite{2003PASP..115..303T} układ \mref{diffeuler}
...
%
W~celu rozwiązania układu równań \mref{rel3}-\mref{rel4} należy policzyć strumienie
...
\mref{eq4} w~kierunku azymutalnym jest następnie wykonywany w~trzech krokach:
%
\begin{enumerate}
...
równanie~\mref{eq:mmsn}, tak, jakby gaz znajdował się w~pionowej równowadze
hydrostatycznej:
%
...
równania~\mref{eq:rho} przyjmuje wartości z~przedziału $[0.4,2]\AU$ dla $R \in
...
\begin{figure}
   ...
   {Przebieg funkcji $f(R)$ opisywanej równaniem~\mref{eq:overlap} dla

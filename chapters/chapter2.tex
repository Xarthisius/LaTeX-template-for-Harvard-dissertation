...
\mathbf{\hat{R}}$.
\par Przyspieszenie będące skutkiem wzajemnego oddziaływania pyłu i gazu przyjmuje postać
...
bezpośrednio prędkości gazu i~pyłu, opisanego w pracy~\cite{TB09}.
...
użyta została tzw. \textit{metoda relaksacji TVD}~\cite{jin-xin-95}, która zapewnia wysoką
dokładność rozwiązań równań hydrodynamicznych oraz numeryczną stabilność, przy
stosunkowo niskich nakładach mocy obliczeniowych. Poniżej przedstawiono krótki \textit{metody relaksacji TVD} na przykładzie równania
...
pomocą róznic skończonych obliczanych metodą \emph{,,pod wiatr''}\footnote{ang.
